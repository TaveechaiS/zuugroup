      ...
      <div className="max-w-5xl mx-auto space-y-5">
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-5">
          <div className="flex items-start justify-between mb-4">
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              <span className="px-3 py-1 bg-yellow-100 text-yellow-700 rounded-full text-xs font-medium">{t('doc.status.pending_review')}</span>
            </div>
          </div>
        ...
        {!showReject ? (
          <div className="flex gap-3 justify-end">
            ...
            <button onClick={() => setShowReject(true)} disabled={saving} className="px-5 py-2.5 border border-red-200 text-red-700 rounded-lg text-sm font-medium hover:bg-red-50 flex items-center gap-2 disabled:opacity-60"><X size={16} /> {t('doc.not_pass')}</button>
            <button onClick={approve} disabled={saving} className="px-5 py-2.5 bg-green-600 hover:bg-green-700 text-white rounded-lg text-sm font-medium flex items-center gap-2 disabled:opacity-60"><Check size={16} /> {saving ? t('doc.saving_dots2') : t('doc.pass_review')}</button>
          </div>
        ) : (
          <div className="bg-white rounded-xl border border-red-200 p-5 space-y-3">
            <p className="text-sm font-medium text-gray-900">{t('doc.reject_reason_fail')}</p>
            ...
            <div className="flex gap-3 justify-end">
              ...
              <button onClick={reject} disabled={saving} className="px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg text-sm font-medium disabled:opacity-60">{saving ? t('doc.saving_dots2') : t('doc.confirm_reject')}</button>
            </div>
          </div>
        )}
      </div>
